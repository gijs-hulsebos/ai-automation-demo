...
export default function LearningRadar(){
  ...
  const [selected,setSelected]=useState(0);const [sourceFilter,setSourceFilter]=useState<'all'|Series>('all');const [enabled,setEnabled]=useState([true,true,true]);
  ...
  const domainDescriptions=lang==='NL'?[
    'Cloudplatformen publiceren, data opslaan en toegang beheren.',
    'Werkprocessen verbinden met agentflows, triggers en automatische verwerking.',
    'AI toepassen in onderzoek, besluitondersteuning en bruikbare applicaties.',
    'Systemen verbinden via API’s, webhooks en gestructureerde gegevens.',
    'Interfaces, tools en applicaties bouwen voor concrete gebruikssituaties.',
    'Gegevens verzamelen, structureren, analyseren en vertalen naar inzichten.',
    'Privacy, toegangscontrole, validatie en verantwoord AI-gebruik.'
  ]:[
    'Deploy cloud platforms, store data and manage access.',
    'Connect work processes through agent flows, triggers and automation.',
    'Apply AI to research, decision support and usable applications.',
    'Connect systems through APIs, webhooks and structured data.',
    'Build interfaces, tools and applications for concrete use cases.',
    'Collect, structure and analyse data to produce useful insights.',
    'Privacy, access control, validation and responsible AI use.'
  ];
  const proofProjects=[{title:'SkillMax+',domain:0,description:lang==='NL'?'Cloud Run · Firestore · activiteitssync':'Cloud Run · Firestore · activity sync'},
   {title:'AI Calendar Assistant',domain:1,description:lang==='NL'?'Agentworkflow · Calendar · Gmail':'Agent workflow · Calendar · Gmail'},
   {title:'Insurance Triage System',domain:2,description:lang==='NL'?'AI-triage · n8n · menselijke review':'AI triage · n8n · human review'}];
  return <section id="learning-journey" className="learning-radar-section" aria-labelledby="learning-radar-title">
    ...
    <div className="learning-radar-panel">
      {!data?<p role="status" className="learning-radar-placeholder">{failed?t.error:t.loading}</p>:<>
        <div className="learning-radar-top"><span title={lang==='NL'?'Laatste geslaagde dashboardverzoek; latency is de gemeten ophaalduur, geen vertraging van GitHub/Vercel-sync.':'Last successful dashboard request; latency measures retrieval, not GitHub/Vercel sync delay.'}>{lang==='NL'?'Opgehaald':lang==='DE'?'Abgerufen':'Fetched'} {retrieval?.time.toLocaleTimeString(locale)} <span className="learning-radar-latency">· {retrieval?.ms} ms</span></span><div className="radar-view-tabs" role="tablist" aria-label="Radar view">{(['knowledge','tech'] as const).map((value,i)=><button key={value} role="tab" id={'radar-tab-'+value} aria-selected={view===value} aria-controls={'radar-panel-'+value} tabIndex={view===value?0:-1} onClick={()=>setView(value)} onKeyDown={e=>{if(['ArrowLeft','ArrowRight','Home','End'].includes(e.key)){e.preventDefault();const next=e.key==='Home'?'knowledge':e.key==='End'?'tech':view==='knowledge'?'tech':'knowledge';setView(next);document.getElementById('radar-tab-'+next)?.focus()}}}>{i?'Tech Stack':lang==='NL'?'Kennis':lang==='DE'?'Wissen':'Knowledge'}</button>)}</div><Link href="/learning-trajectory" className="learning-radar-connected">{t.title}<ArrowUpRight size={13}/></Link></div>
        <div className="learning-radar-summary">{keys.map((k,s)=><div key={k}><span style={{color:colors[s]}}>{t.series[s]}</span><strong>{data.totals[k].toLocaleString(locale)}</strong><small>{units[s]}</small></div>)}</div><div role="tabpanel" id="radar-panel-knowledge" aria-labelledby="radar-tab-knowledge" hidden={view!=='knowledge'} className="radar-view-panel"><div className="knowledge-profile-intro"><div><span>APPLIED AI ENGINEERING</span><h3>{lang==='NL'?'Van AI-model tot werkende applicatie.':'From AI model to working application.'}</h3><p>{lang==='NL'?'Cloud, automatisering en software — onderbouwd met projecten en afgeronde cursussen.':'Cloud, automation and software — backed by projects and completed courses.'}</p></div><div className="knowledge-proof-projects">{proofProjects.filter(p=>data.evidence.some(e=>e.series==='practice'&&e.title===p.title)).map(p=><button key={p.title} onClick={()=>{setSelected(p.domain);setSourceFilter('practice')}}><small>{t.labels[p.domain]}</small><strong>{p.title}<ArrowUpRight size={14}/></strong><span>{p.description}</span></button>)}</div></div><div className="learning-radar-body"><div className="learning-radar-plot">
        ...
        </div><aside className="learning-radar-inspector"><div className="learning-radar-domain-buttons">{data.axes.map((a,i)=><button key={a.key} onMouseEnter={()=>setSelected(i)} onFocus={()=>setSelected(i)} onClick={()=>setSelected(i)} aria-pressed={selected===i}>{t.labels[i]}</button>)}</div><h3>{t.labels[selected]}</h3><p className="knowledge-domain-description">{domainDescriptions[selected]}</p>{keys.map((k,s)=><div className="learning-radar-value" key={k}><span><i style={{background:colors[s]}}/>{t.series[s]}</span><strong>{data.axes[selected][k].toLocaleString(locale)}</strong></div>)}<div className="knowledge-source-tabs">{(['all','practice','theory'] as const).map(k=><button key={k} aria-pressed={sourceFilter===k} onClick={()=>setSourceFilter(k)}>{k==='all'?(lang==='NL'?'Alle bronnen':'All sources'):k==='practice'?(lang==='NL'?'Projecten':'Projects'):(lang==='NL'?'Cursussen':'Courses')}</button>)}</div><div className="learning-radar-evidence">{data.axes[selected].evidence.filter(e=>sourceFilter==='all'||e.series===sourceFilter).map((e,i)=><div key={i}><i style={{background:colors[keys.indexOf(e.series)]}}/><span><a href={e.url}>{e.title} ↗</a><small>{e.detail}</small></span><b>{e.count}</b></div>)}{!data.axes[selected].evidence.length&&<p>{lang==='NL'?'Nog geen ingedeelde registraties.':lang==='DE'?'Noch keine zugeordneten Einträge.':'No classified records yet.'}</p>}</div></aside></div>
