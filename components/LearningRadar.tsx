...
const copy={
  NL:{title:'Leertraject',headline:'Van kennis naar uitvoering.',intro:'Theorie, praktijk en oefenen — verbonden met mijn leertraject in SkillMax+.',labels:['Cloud','Hosting','Software','AI & data','Automatisering','Security','Overig'],series:['Theorie','Praktijk','Oefenen'],open:'Bekijk het leertraject',loading:'Leeractiviteit ophalen…',error:'Leerdata is tijdelijk niet beschikbaar.',stale:'Vernieuwen is niet gelukt. De laatst opgehaalde data blijft zichtbaar.',detail:'Geregistreerde activiteit',note:'Logaritmische schaal: 1, 10, 100… registraties. Dit toont activiteit, geen vaardigheidsniveau.',method:'Cursussen worden op titel ingedeeld. Commits vallen onder Software, deployments onder Hosting en cloudregistraties onder Cloud. Zonder herkenbaar vakgebied valt activiteit onder Overig. Planning telt niet mee.',zero:'Geen oefeningen geregistreerd in deze periode.',updated:'Bron opgehaald',unit:'registraties'},
  EN:{title:'Learning journey',headline:'From knowledge to execution.',intro:'Theory, practice and exercises — connected to my learning journey in SkillMax+.',labels:['Cloud','Hosting','Software','AI & data','Automation','Security','Other'],series:['Theory','Practice','Exercises'],open:'Explore the learning journey',loading:'Loading learning activity…',error:'Learning data is temporarily unavailable.',stale:'Refresh failed. Showing the last retrieved data.',detail:'Recorded activity',note:'Logarithmic scale: 1, 10, 100… records. This shows activity, not proficiency.',method:'Courses are classified by title. Commits map to Software, deployments to Hosting and cloud records to Cloud. Records without a known subject remain Other. Plans are excluded.',zero:'No exercises recorded in this period.',updated:'Source retrieved',unit:'records'},
  DE:{title:'Lernweg',headline:'Von Wissen zu Umsetzung.',intro:'Theorie, Praxis und Übungen — verbunden mit meinem Lernweg in SkillMax+.',labels:['Cloud','Hosting','Software','KI & Daten','Automation','Security','Sonstiges'],series:['Theorie','Praxis','Übungen'],open:'Lernweg ansehen',loading:'Lernaktivität wird geladen…',error:'Lerndaten sind vorübergehend nicht verfügbar.',stale:'Aktualisierung fehlgeschlagen. Die zuletzt geladenen Daten bleiben sichtbar.',detail:'Erfasste Aktivität',note:'Logarithmische Skala: 1, 10, 100… Einträge. Zeigt Aktivität, keine Kompetenzbewertung.',method:'Kurse werden nach Titel eingeordnet. Commits zählen zu Software, Deployments zu Hosting und Cloud-Einträge zu Cloud. Ohne bekanntes Fachgebiet bleibt Aktivität unter Sonstiges. Planung zählt nicht mit.',zero:'Keine Übungen in diesem Zeitraum erfasst.',updated:'Quelle abgerufen',unit:'Einträge'},
};
  ...
  return <section id="learning-journey" className="learning-radar-section" aria-labelledby="learning-radar-title">
    ...
    <div className="learning-radar-panel">
      {!data?<p role="status" className="learning-radar-placeholder">{failed?t.error:t.loading}</p>:<>
        <div className="learning-radar-top"><span>{date(data.from)} — {date(data.to)}</span><span className="learning-radar-connected">SkillMax+ · Cloud</span></div>
        <div className="learning-radar-body"><div className="learning-radar-plot">
          <svg viewBox="0 0 600 550" role="img" aria-label={`${t.title}: ${t.note}`}>
            ...
            {keys.map((key,s)=>enabled[s]&&<g key={key}><polygon points={data.axes.map((a,i)=>point(i,radius(a[key])).join(',')).join(' ')} fill={colors[s]} fillOpacity=".10" stroke={colors[s]} strokeWidth="2" strokeLinejoin="round"/>{data.axes.map((a,i)=>a[key]>0&&<circle key={i} cx={point(i,radius(a[key]))[0]} cy={point(i,radius(a[key]))[1]} r="4" fill={colors[s]} stroke="#101013" strokeWidth="2"/>)}</g>)}
