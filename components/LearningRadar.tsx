...
export default function LearningRadar(){
  ...
  const [selected,setSelected]=useState(-1);const [enabled,setEnabled]=useState([true,true,true]);
  useEffect(()=>{const controller=new AbortController();let busy=false;
    const update=async()=>{if(busy)return;busy=true;const started=performance.now();try{const r=await fetch('/api/learning-radar',{signal:controller.signal,cache:'no-store'});if(!r.ok)throw Error();const value=await r.json();if(!controller.signal.aborted){setData(value);setRetrieval({time:new Date(),ms:Math.round(performance.now()-started)});setFailed(false)}}catch{if(!controller.signal.aborted)setFailed(true)}finally{busy=false}};
    void update();const timer=setInterval(()=>{if(document.visibilityState==='visible')void update()},15000);
    const onVisible=()=>{if(document.visibilityState==='visible')void update()};document.addEventListener('visibilitychange',onVisible);window.addEventListener('focus',onVisible);return()=>{controller.abort();clearInterval(timer);document.removeEventListener('visibilitychange',onVisible);window.removeEventListener('focus',onVisible)};
  },[]);
  ...
  const inspected=selected<0&&data?{...data.totals,evidence:data.evidence}:chartAxes[selected];
  ...
  return <section id="learning-journey" className="learning-radar-section" aria-labelledby="learning-radar-title">
    ...
    <div className="learning-radar-panel">
      {!data?<p role="status" className="learning-radar-placeholder">{failed?t.error:t.loading}</p>:<>
        ...
        </div><aside className="learning-radar-inspector"><div className="learning-radar-domain-buttons"><button onClick={()=>setSelected(-1)} aria-pressed={selected===-1}>All</button>{chartAxes.map((a,i)=><button key={a.key} onMouseEnter={()=>setSelected(i)} onFocus={()=>setSelected(i)} onClick={()=>setSelected(i)} aria-pressed={selected===i}>{t.labels[i]}</button>)}</div><h3>{selected<0?'All':t.labels[selected]}</h3>{keys.map((k,s)=><div className="learning-radar-value" key={k}><span><i style={{background:colors[s]}}/>{t.series[s]}</span><strong>{inspected[k].toLocaleString(locale)}</strong></div>)}<small>{t.unit}</small><div className="learning-radar-evidence">{inspected.evidence.map((e,i)=><div key={i}><i style={{background:colors[keys.indexOf(e.series)]}}/><span><a href={e.url}>{e.title} ↗</a><small>{e.detail}</small></span><b>{e.count}</b></div>)}{!inspected.evidence.length&&<p>{lang==='NL'?'Nog geen ingedeelde registraties.':lang==='DE'?'Noch keine zugeordneten Einträge.':'No classified records yet.'}</p>}</div></aside></div>
