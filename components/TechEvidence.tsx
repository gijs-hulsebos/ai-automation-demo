...
export default function TechEvidence({records,lang}:{records:EvidenceRecord[];lang:'NL'|'EN'|'DE'}){
 ...
 const axes=nodes;
 const limit=Math.max(8,Math.ceil(Math.max(0,...axes.flatMap(n=>keys.map(k=>n.counts[k])))/8)*8);
 const radius=(count:number)=>195*Math.sqrt(count/limit);
 ...
 const uniqueRecords=[...new Map(records.map(e=>[e.id,e])).values()];
 const all:TechnologyNode={id:'all',name:'All',group:'all',counts:{theory:0,practice:0,exercises:0},evidenceIds:uniqueRecords.map(e=>e.id),moduleIds:[...new Set(nodes.flatMap(n=>n.moduleIds))],versions:[],strength:uniqueRecords.length};
 for(const record of uniqueRecords)all.counts[record.series]+=record.count;
 const active=nodes.find(n=>n.id===selected)||all;
 ...
 return <div className="tech-view">
  <div className="tech-layout"><div className="tech-radar-map">
   <svg viewBox="0 0 600 550" role="img" aria-label="Tech Stack radar">
    {rings.map(value=><g key={value}><polygon points={Array.from({length:Math.max(3,axes.length)},(_,i)=>point(i,radius(value)).join(',')).join(' ')} fill="none" stroke="#ffffff20" strokeDasharray="3 5"/><text x="308" y={275-radius(value)+12} fill="#777780" fontSize="10">{value}</text></g>)}
    ...
    {keys.map((key,i)=><g key={key}><polygon points={Array.from({length:Math.max(3,axes.length)},(_,j)=>point(j,radius(axes[j]?.counts[key]||0)).join(',')).join(' ')} fill={colors[i]} fillOpacity=".21" stroke={colors[i]} strokeWidth="3" strokeLinejoin="round"/>{axes.map((node,j)=>node.counts[key]>0&&<circle key={node.id} cx={point(j,radius(node.counts[key]))[0]} cy={point(j,radius(node.counts[key]))[1]} r="4" fill={colors[i]} stroke="#101013" strokeWidth="2"/>)}</g>)}
